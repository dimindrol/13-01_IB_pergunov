...
1. SYN Scan (-sS)  
Сканирование SYN часто называют “полуоткрытым” или “скрытым” сканированием. SYN-сканирование работает так же, как сканирование TCP Connect с закрытыми и отфильтрованными портами, т. Е. Получает ПЕРВЫЙ пакет для закрытого порта и не отвечает для отфильтрованных портов.  
2. FIN Scan (-sF)  
Отправляет FIN-пакеты. Если порт закрыт, сервер отвечает RST. Если порт открыт, ответа обычно не будет.  
3. Xmas Scan (-sX)  
...
1. SYN Scan (-sS)  
Ответ SYN-ACK указывает на открытый порт, RST указывает на закрытый порт.  
2. FIN Scan (-sF)  
Отсутствие ответа обычно указывает на открытый порт, RST указывает на закрытый порт.  
3. Xmas Scan (-sX)  
Подобно FIN Scan, отсутствие ответа указывает на открытый порт, RST указывает на закрытый порт.  
4. UDP Scan (-sU)  
